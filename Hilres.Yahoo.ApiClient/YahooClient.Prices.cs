...
public sealed partial class YahooClient
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (firstDate != null && lastDate != null)
        {
            ...
            {
                throw new ArgumentException($"{nameof(firstDate)} ({firstDate}) is greater then {nameof(lastDate)} ({lastDate})");
            }
        }
        ...
        if (response == null)
        {
            throw new Exception($"{nameof(this.apiHttpClient.GetAsync)} return null.");
        }
